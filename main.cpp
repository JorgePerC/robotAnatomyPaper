...
int main(int argc, const char * argv[]) {
    ...
    // Add initialization prior to goto
    std::chrono::steady_clock::time_point start_time;
    std::chrono::steady_clock::time_point now;

    sl_u16 modeID = 1; // 0 = Standard, 1 = Express, 2 = Boost 
    ...
    for (auto& mode : scanModes) {
        printf("Mode ID: %d  Name: %s  SampleRate: %.1f sps  MaxDist: %.1f m\n",
            mode.id, mode.scan_mode, 1000000.0f / mode.us_per_sample, mode.max_distance);
    }

    /*
    // Find out which mode 'typical' resolves to
    drv->startScan(0, 1, 0, &currentMode);
    printf("Using scan mode: %s (ID: %d)\n", currentMode.scan_mode, currentMode.id);
    
    // start scan...
    drv->startScan(0,1);
    */

    drv->startScanExpress(false, modeID, 0, &currentMode);
    printf("Scanning in mode: %s  SampleRate: %.1f sps\n",
        currentMode.scan_mode, 1000000.0f / currentMode.us_per_sample);
    ...
    while (1) {
        ...
        if (SL_IS_OK(op_result)) {
            ...
            printf("Scan received at time: %lld ms\n",
                   (long long)timestamp_ms);

            for (int pos = 0; pos < (int)count; ++pos) {

                printf("%s theta: %03.2f Dist: %08.2f Q: %d\n",
                    (nodes[pos].flag & SL_LIDAR_RESP_HQ_FLAG_SYNCBIT)
                        ? "S "
                        : "  ",
